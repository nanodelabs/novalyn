...
pub fn last_tag(repo: &Repository) -> Result<Option<String>, git2::Error> {
    ...
    for name_opt in tags.iter() {
        let name = match name_opt { Some(n) => n, None => continue };
...
pub fn current_ref(repo: &Repository) -> Result<Option<String>, git2::Error> {
    let head = match repo.head() { Ok(h) => h, Err(e) => return if e.code() == git2::ErrorCode::UnbornBranch { Ok(None) } else { Err(e) } };
    if head.is_branch() { return Ok(head.shorthand().map(|s| s.to_string())); }
    ...
    if let Some(oid) = oid {
        ...
        for name_opt in tags.iter() {
            if let Some(name) = name_opt {
                if let Ok(tag_oid) = repo.refname_to_id(&format!("refs/tags/{}", name)) {
                    if tag_oid == oid { return Ok(Some(name.to_string())); }
                }
            }
        }
    }
    Ok(Some(format!("DETACHED@{}", oid.map(|o| o.to_string()).unwrap_or_default())))
}

pub fn commits_between(repo: &Repository, from: Option<&str>, to: &str) -> Result<Vec<RawCommit>, git2::Error> {
    ...
    if let Some(from_ref) = from {
        if let Ok(from_obj) = repo.revparse_single(from_ref) {
    ...
    for oid_res in revwalk {
        if let Ok(oid) = oid_res { if let Ok(commit) = repo.find_commit(oid) { commits.push(to_raw_commit(&commit)); } }
    }
...
fn to_raw_commit(commit: &git2::Commit) -> RawCommit {
    ...
    RawCommit { id, short_id, summary, body, author_name: sig.name().unwrap_or("").to_string(), author_email: sig.email().unwrap_or("").to_string(), timestamp: time.seconds() }
}
...
pub fn is_dirty(repo: &Repository, include_untracked: bool) -> Result<bool, git2::Error> {
    ...
    for entry in statuses.iter() {
        let s = entry.status();
        if s.intersects(git2::Status::WT_MODIFIED | git2::Status::WT_DELETED | git2::Status::WT_RENAMED | git2::Status::WT_TYPECHANGE | git2::Status::INDEX_NEW | git2::Status::INDEX_MODIFIED | git2::Status::INDEX_DELETED | git2::Status::INDEX_RENAMED | git2::Status::INDEX_TYPECHANGE | git2::Status::WT_NEW) {
            return Ok(true);
        }
    }
    Ok(false)
...
pub fn add_and_commit(repo: &Repository, message: &str) -> Result<Oid, git2::Error> {
    ...
    let parent_commits: Vec<git2::Commit> = match repo.head() {
        Ok(head) => {
            if let Some(oid) = head.target() { vec![repo.find_commit(oid)?] } else { vec![] }
        },
        Err(_) => vec![],
    };
...
pub fn create_tag(repo: &Repository, name: &str, message: &str, annotated: bool) -> Result<Oid, git2::Error> {
    let head = repo.head()?;
    let target = head.peel(git2::ObjectType::Commit)?;
    let commit = target.into_commit().map_err(|_| git2::Error::from_str("HEAD is not a commit"))?;
    let sig = repo.signature()?;
    if annotated {
        repo.tag(name, commit.as_object(), &sig, message, false)
    } else {
        repo.reference(&format!("refs/tags/{}", name), commit.id(), false, message).map(|r| r.target().unwrap())
    }
}
